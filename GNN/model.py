...
class GNNEncoder(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, edge_dim):
        ...
        self.conv1 = GATv2Conv((-1, -1), hidden_channels, edge_dim=edge_dim, add_self_loops=False)  # Aggregates information from the node's 1-hop neighbors
        self.conv2 = GATv2Conv(hidden_channels, hidden_channels, edge_dim=edge_dim, add_self_loops=False)  # Aggregates information from the node's 2-hop neighbors
        self.conv3 = GATv2Conv(hidden_channels, hidden_channels, edge_dim=edge_dim, add_self_loops=False)  # Aggregates information from the node's 3-hop neighbors
        self.conv4 = GATv2Conv(hidden_channels, hidden_channels, edge_dim=edge_dim, add_self_loops=False)  # Aggregates information from the node's 4-hop neighbors
        self.conv5 = GATv2Conv(hidden_channels, out_channels, edge_dim=edge_dim, add_self_loops=False)  # Aggregates information from the node's 5-hop neighbors
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x = self.conv3(x, edge_index, edge_attr).relu()  # Update embeddings with 3-hop neighbors
        x = self.conv4(x, edge_index, edge_attr).relu()  # Update embeddings with 4-hop neighbors
        x = self.conv5(x, edge_index, edge_attr)  # Update embeddings with 5-hop neighbors (final output dimension)
